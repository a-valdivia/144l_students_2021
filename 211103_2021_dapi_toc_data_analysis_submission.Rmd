...
A1: The GNP treatment shows the largest increase in cell abundance over the course of the experiment, followed by the Kelp Exudate + NP, Kelp Exudate, and lastly the Control which had the smallest increase. The oxygen drawdown data shows the largest oxygen drawdown from the GNP treatment and mostly follows the same patterns for the other treatments. This makes sense as over the course of the experiment, the bacteria are consuming oxygen and building biomass, therefore leading to decreased oxygen levels and increased cell abundance. 
However, the Kelp Exudate + NP shows a slightly smaller oxygen drawdown than the Kelp Exudate treatment even though the Kelp Exudate + NP has a higher cell abundance. Several hypotheses come to mind for the discrepancy between cell abundance and oxygen drawdown for these two treatments. Perhaps the Kelp Exudate + NP is more efficient at using the oxygen it takes in and therefore requires less oxygen to complete similar tasks. For example, perhaps the added nitrate and phosphate could serve as resources that allow the cell to take more efficient biosynthesis pathways that don't require as much of an input of resources. Alternatively, this could be due to the bacterial community composition. Perhaps there was a bacterial species that dominated the Kelp Exudate + NP that required less oxygen to grow. 
...
A2: The cell abundance data shows a slight decrease after Day 2 (time point 4) in the GNP treatment while the biovolume data has a much steeper decrease after the same point. 
In the Kelp + NP treatment there is also a much steeper decrease in the biovolume data but additionally, the biovolume increases to a higher volume than any of the other treatments. This is different compared to the cell abundance data in which it had only the second largest increase.
In the Kelp Exudate treatment, the same pattern was observed as the GNP treatment where there is a much sharper decrease in the biovolume data after Day 2 compared to the cell abundance data. 
The control stays nearly at the same mean biovolume (with a slight decrease after Day 2) whereas it followed a linear increase in the cell abundance data. 
...
A3: The DAPI data was only collected at 3 timepoints for the duration of the experiment whereas the FCM data was collected at all 8 timepoints. This is a limitation because the DAPI data is a smaller field of information to draw conclusions from. Particularly when calculating specific growth rates from constructed bacterial growth curves, the goal is to identify where exponential growth is occurring. The "curve" from the DAPI data will be constructed from three points whereas more data points (such as in the FCM data) can give a more accurate depiction of which region contains exponential growth.
...
A4: There are similar patterns in both the bacterial carbon value data graphs. For example, the patterns across the treatments tend to be similar; they show an increase in bacterial carbon from 0-2 days and a decrease from 2-4 days. The values all relatively increase, however, when accounting for cell volume as noted by the scale of the y-axis in both graphs. Looking at the GNP treatment in the non-biovolume adjusted graph, the peak of the curve (at Day 2) reaches a value of ~4.5 umol C L^-1 whereas in the biovolume adjusted graph it reaches nearly 7 umol C L^-1. When accounting for biovolume, the bacterial carbon amounts are higher across all the treatments. 
...
A5: Yes, the patterns in these data align with the previous observations of cell abundance and oxygen drawdown. The GNP treatment had the largest ∆Cells, highest specific growth rate (u), and the shortest doubling time. This was followed by Kelp Exudate + NP, Kelp Exudate, and the Control treatment which had the smallest ∆Cells, the lowest specific growth rate (u), and the longest doubling time. This aligns with the idea that the cells in the GNP treatment grew rapidly in the favorable environment leading to the largest and fastest increase in cells.
...
A6: a) The target concentration of carbon is 10uM (micromolar).
    b) No, our target was not hit in each treatment. We came the closest in the GNP treatment, but in the other treatments we fell short of the goal. 
    c) This impairs our ability to assess the effect of the **type** of DOC added because now there is the added variable of the **amount** of TOC added. In order to assess the results comprehensively, we must take into account how the differences in starting TOC amounts may have affected the results. This is especially important as we observed the highest cell abundance, oxygen drawdown, specific growth rate, ∆cells, and shortest doubling time in the GNP treatment which also had the highest starting amount of TOC. Therefore, this data could be a reflection of either the type of DOC or the amount, or both.  
    d) Over time, TOC across all the incubation treatments decreases. This aligns with our other data showing that the cells were taking up the resources and using it to build biomass. 
...
A7: Incorporating the biovolume does not change the interpretation of the ∆DOC too much. Looking at the graphs a and b, the patterns and scale hardly change at all. There is a slight change, however, between the Kelp Exudate + NP and the Kelp Exudate group. With the biovolume adjusted data the Kelp Exudate + NP has a larger ∆DOC whereas it has a slightly smaller change in the non-adjusted data. 
Incorporating the biovolume does not change the interpretation of the Bioavailable DOC values. The patters of the treatments and the scale of the values remain nearly the same between both graphs. Again, there is the small switch where the Kelp Exudate + NP has a higher bioavailable DOC fraction compared to teh Kelp Exudate treatment when biovolume is accounted for, but a lower fraction when biovolume is ignored. 
Incorporating the biovolume does change the interpretation of the BGE values. The patterns across the treatments largely remains the same, however, the control has the lowest BGE in the biovolume adjusted graph relative to the other treatments whereas it had the second highest BGE in the non-biovolume-adjusted data. Additionally, the scale of the graphs do not align, the BGE across most of the treatments is much higher in the biovolume derived graph. For example, in the Kelp Exudate+NP treatment, the BGE is nearly double in the biovolume graph. The control value stays near the same across both graphs. 